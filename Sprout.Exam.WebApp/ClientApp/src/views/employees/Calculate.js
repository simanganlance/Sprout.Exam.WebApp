...
export class EmployeeCalculate extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = { id: 0,fullName: '',birthdate: '',tin: '', formattedBirthDate: '',typeId: 1,absentDays: 0,workedDays: 0,netIncome: 0, salary: 0, loading: true,loadingCalculate:false};
  }
...
<div className='form-row'>
<div className='form-group col-md-12'>
  <label >Birthdate: <b>{this.state.formattedBirthDate}</b></label>
</div>
</div>
...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
  ...
  async getEmployee(id) {
    ...
    if(response.status === 200){
        const data = await response.json();
        this.setState({ id: data.id,fullName: data.fullName,birthdate: data.birthdate, formattedBirthDate: data.formattedBirthDate, tin: data.tin,typeId: data.typeId,salary: data.salary,loading: false,loadingCalculate: false });
    }
    else{
        const errorResponseText = await response.text();
        alert(errorResponseText);
